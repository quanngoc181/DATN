$(function() {
	...
	$('a[data-toggle="pill"]').on('shown.bs.tab', function(e) {
		let type = +$(this).closest('.nav-item').data('type');
		let status = +$(this).closest('.nav-item').data('status');
		let keyword = $('.tab-pane.active').find('.search-input').val();
		reRender(type, status, keyword);
	});

	$(document).on('keyup', '.search-input', debounce(function() {
		let type = +$(this).closest('.tab-pane').data('type');
		let status = +$(this).closest('.tab-pane').data('status');
...
function reRender(type, status, keyword) {
	$.ajax({
		url: "/admin/order-management/fetchOrder",
		data: {
			type: type,
			status: status,
			keyword: keyword
